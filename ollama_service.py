...
def truncate_middle(chat_history: List[Dict[str, str]]):
    ...
    if to_remove % 2 != 0:
        to_remove += 1
    logging.debug(
        f"Truncating {to_remove} messages from a random place in chat history, ensuring the last element is preserved"
    )
    if to_remove > 0:
        start_index = max(2, random.randrange(2, len(chat_history) - to_remove - 1, 2))
        chat_history = (
            chat_history[:start_index] + chat_history[start_index + to_remove :]
        )
        logging.warning(
            f"Truncated chat history to {len(chat_history)} messages starting at {start_index} removing {to_remove} messages"
        )
...
def handle_client(
    ...
    while keep_looping:
        try:
            ...
            if any(
                data.lower().startswith(cmd) for cmd in {"/stop", "/quit", "/exit"}
            ) or any(data.lower().endswith(cmd) for cmd in {"/stop", "/quit", "/exit"}):
            ...
            if byte_size > max_bytes - 1024:
                logging.warning(
                    f"Chat history is too long {len(chat_history)} ({byte_size} bytes), truncating"
                )
                chat_history, start_index, to_remove = truncate_middle(chat_history)
                prefix = (
                    f"/truncated: {to_remove} messages starting at {start_index}<br>"
                )
            else:
                logging.debug(
                    f"Chat history is {len(chat_history)} messages ({byte_size} bytes of max {max_bytes} bytes)"
                )
                prefix = ""
            ...
            logging.debug(
                f"{response['prompt_eval_count']} input tokens evaluated in {round(response['prompt_eval_duration']/1e9, 3)} seconds {round(1e9 * float(response['prompt_eval_count']) / response['prompt_eval_duration'], 3)} tokens/sec, "
    ...
    if not keep_looping:
        logging.info(
            f"Terminating conversation after receiving /bye, closing connection ({s})"
        )
        if s.fileno() != -1:
            s.sendall("/end".encode("utf-8"))
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        "--debug",
        action="store_true",
        help="Enable debug mode with more extensive logging, also to console",
    )
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            ...
            handle_client(
                s, ollama_client, config, system_prompt, persona_name, args.quiet
            )
