...
def handle_client(
    ...
    msg_count = 0
    max_bytes = (
        config["max_tokens"] * 6 if "max_tokens" in config else 32768
        ...
        try:
            ...
            if data.lower().startswith("/end") or data.lower().endswith("/end"):
                ...
                try:
                    ...
                    logging.info(
                        f"Socket already closed by partner when sending /end: {e}"
                    )
                break
            ...
            )  # remove the start sequence from the prompt
...
def main():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        try:
            ...
            handle_client(
                s, openai_client, config, system_prompt, persona_name, args.quiet
            )
        except socket.error as e:
