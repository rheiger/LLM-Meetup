...
def main():
    ...
    parser.add_argument(
        "-c",
        "--config",
        default="config/llm_service_caller_cconfig.yml",
        help="YAML configuration file",
    )
    parser.add_argument(
        "-s",
        "--service",
        choices=["lmstudio", "openai", "ollama", "anthropic"],
        help="Select the service to use",
    )
    ...
    if not args.service:
        parser.error(
            "Service must be specified either in the config file or as a command-line argument"
        )
